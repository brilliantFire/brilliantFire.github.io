...
TMM is considered a good general method for between-sample normalization. However, we can only be confident in its results if the assumptions of the method are satisfied. For TMM, the assumption is that *the majority of genes are __NOT__ differentially expressed*. Other methods have other assumptions.
